...
from src.ai.schemas import (
    ...
    WhiteboardOutput, AudioScriptOutput, MirrorQuestion,
)

logger = logging.getLogger(__name__)
MODEL_RICH  = "claude-haiku-4-5-20251001"
MODEL_FAST  = "claude-haiku-4-5-20251001"
MAX_TOKENS_WB = 4096   # Whiteboard necesita espacio para JSON completo
MAX_TOKENS    = 900    # Mirror y Audio
...
class AIOrchestrator:
    ...
    async def run_help_session(self, request: AIHelpRequest) -> AIHelpResponse:
        ...
        # 2 llamadas en paralelo: pizarra + pregunta espejo
        wb, mirror = await asyncio.gather(
            self._whiteboard(request, sid),
            self._mirror(request, sid),
        )

        # 1 llamada más: guion de audio (usa contenido de la pizarra)
        audio_sc = await self._audio_script(request, wb, sid)

        audio_b64 = await self._tts(audio_sc, request.student_gender, sid)
        ms = int((time.time() - t0) * 1000)
        logger.info(f"[{sid}] OK en {ms}ms — 3 llamadas Claude")

        clf = ICFESClassification(area=request.area or "")
    ...
    # ── Pizarra ───────────────────────────────────────────────────────────────
    async def _whiteboard(self, req, sid):
        r = await self._llm(
            self.registry.get("task.whiteboard-generator"),
            f"Genera la pizarra acrílica.\n\nPREGUNTA:\n{req.question_text}\n\n"
            f"OPCIONES:\n{self._fmtopts(req.options)}\n\n"
            f"ÁREA: {req.area or 'Matemáticas'}\n\nDevuelve SOLO JSON válido.",
            max_tokens=MAX_TOKENS_WB,
        )
        data = self._json(r, sid, "whiteboard")
        clf  = ICFESClassification(area=req.area or "")
        plan = SocraticPlan()
        data = self._complete_whiteboard_data(req, clf, plan, data)
        return WhiteboardOutput(**data)

    # ── Guion de audio ────────────────────────────────────────────────────────
    async def _audio_script(self, req, wb, sid):
        bloques = "\n".join(
            f"- {b.title}: {b.content[:80]}"
            for b in (wb.blue_reasoning or [])[:3]
        )
        r = await self._llm_fast(
            self.registry.get("task.audio-script-generator"),
    ...
    # ── Pregunta espejo ───────────────────────────────────────────────────────
    async def _mirror(self, req, sid):
        r = await self._llm_fast(
            self.registry.get("task.mirror-question-generator"),
    ...
    # ── Llamadas LLM ──────────────────────────────────────────────────────────
    async def _llm(self, system: str, user: str, max_tokens: int = MAX_TOKENS_WB) -> str:
        msg = await self.claude.messages.create(
            model=MODEL_RICH, max_tokens=max_tokens,
            system=system, messages=[{"role":"user","content":user}])
        return msg.content[0].text

    async def _llm_fast(self, system: str, user: str, max_tokens: int = MAX_TOKENS) -> str:
        msg = await self.claude.messages.create(
    ...
    def _json(self, text: str, sid: str, step: str) -> dict:
        ...
        try:
            ...
            try:
                ...
                if extra_chars:
                    logger.warning(f"[{sid}] JSON en {step} traía {extra_chars} chars extra; ignorados.")
    ...
    def _complete_whiteboard_data(self, req, clf, plan, data: dict) -> dict:
            ...
            options = [
                {
                    ...
                    "status": "review",
                    "reason": f"Contrasta esta opción con el criterio azul: {self._short(o['text'] if isinstance(o, dict) else o.text)}",
                }
        ...
        # Validar audio_sync_markers: solo aceptar dicts bien formados
        raw_markers = data.get("audio_sync_markers") or []
        valid_markers = [
            m for m in raw_markers
            if isinstance(m, dict) and "time_seconds" in m and isinstance(m.get("time_seconds"), (int, float))
        ]
        audio_sync_markers = valid_markers or [
    ...
    def _ensure_formula_steps(self, formula: dict | None, blue: list[dict], req) -> dict:
        ...
        if not steps:
            ...
            steps.append("PROCESO: Identifica la fórmula que aplica, sustituye los valores y opera con orden.")
            steps.append("VERIFICACIÓN: Revisa si el resultado tiene sentido y la unidad correcta.")
    ...
    def _default_red_blocks(self, req, subject: str, plan) -> list[dict]:
        planned = self._blocks_from_texts(
            plan.what_to_avoid[:2],
            ["Trampa de lectura", "Distractor típico"],
            warning=True
        )
        if len(planned) >= 1:
            return planned[:1]
        ...
        subject_key = self._subject_key(subject, req.question_text)
        templates = {
            "geometria":    [("Error más común", "No confundas perímetro (suma de lados) con área (superficie). Usa la fórmula correcta para lo que pide el enunciado.")],
            "matematicas":  [("Error más común", "Respetar jerarquía de operaciones: primero paréntesis, luego potencias, luego × y ÷, finalmente + y −.")],
            "quimica":      [("Error más común", "Leer la flecha al revés invierte reactivos y productos. Identifica bien qué está antes y qué está después de →.")],
            "fisica":       [("Error más común", "Usar la fórmula equivocada por las unidades incorrectas. Verifica que los datos correspondan a la magnitud que pide la ley.")],
            "biologia":     [("Error más común", "Confundir nivel biológico: célula, tejido, órgano y ecosistema no explican los mismos fenómenos.")],
            "lectura":      [("Error más común", "Elegir una opción con palabras del texto sin verificar que responda la pregunta. La evidencia textual debe justificar la respuesta.")],
            "ingles":       [("Error más común", "Traducir literalmente en vez de analizar el contexto gramatical y comunicativo de la oración.")],
            "sociales":     [("Error más común", "Atribuir causas o consecuencias a épocas equivocadas. Contextualiza el hecho histórico o social antes de responder.")],
        }
        return [
            {"order": 1, "title": title, "content": content, "visual_warning": ""}
            for title, content in templates.get(subject_key, templates["matematicas"])
        ]
    ...
    def _infer_board_title(self, question_text: str, subject: str) -> str:
        ...
        if subject_key == "geometria":  return "Geometría: dibuja, marca y verifica la figura"
        if subject_key == "fisica":     return "Física: sistema, magnitudes y sentido"
        if subject_key == "biologia":   return "Biología: proceso, causa y evidencia"
        if subject_key == "lectura":    return "Lectura crítica: evidencia e inferencia"
        if subject_key == "ingles":     return "Inglés: contexto, gramática y coherencia"
        if subject_key == "sociales":   return "Sociales: contexto, causa y consecuencia"
    ...
    def _count_atoms(self, formula_str: str) -> list[list]:
    ...
    def _default_visual_elements(self, req, subject: str, formula: dict | None) -> list[dict]:
        ...
        if formula and formula.get("reactants") and formula.get("products"):
            ...
            visuals.extend([
                ...
                {
                    "type": "comparison_table",
                    "title": "¿Qué pide el enunciado?",
                    "headers": ["Medida", "Pregunta clave", "Riesgo"],
                    "rows": [
                        ["Longitud", "¿Pide lado, radio o altura?", "No es área"],
            ...
            visuals.extend([
                ...
                {
                    "type": "comparison_table",
                    "title": "Magnitudes del problema",
                    "headers": ["Dato", "Valor", "Unidad", "Rol"],
                    "rows": [["—", "—", "—", "Identifica en el enunciado"]],
                },
            ])
    ...
    def _valid_visuals(self, value: object) -> list[dict]:
        ...
        for item in value:
            ...
            useful = any(item.get(k) for k in content_keys)
    ...
    def _valid_options(self, value: object) -> list[dict]:
        ...
        for item in value:
            ...
            if option and reason:
                valid.append({**item, "option": option, "reason": reason,
                              "status": self._clean_text(item.get("status")) or "review"})
        return valid

    def _is_blank_formula(self, formula: object) -> bool:
        if not isinstance(formula, dict):
            return True
        return not any(formula.get(k) for k in ("type","equation","reactants","products","variables","steps"))
    ...
    def _is_generic_subject(self, subject: str) -> bool:
        return self._clean_text(subject).lower() in {"", "icfes", "saber 11", "prueba icfes", "general", "razonamiento"}

    def _is_generic_title(self, title: str) -> bool:
        return self._clean_text(title).lower() in {
            "", "pizarra acrilica de razonamiento", "pizarra acrílica de razonamiento",
            "mapa visual del problema", "icfes",
        }
